...
int parseFile(const char* fileName) {
	...
	while (fgets(line, sizeof(line), ip)) {
		...
		while (token != NULL) {
			if (tflag == 0) { 	// rollno
			    // validation checking
			     if (!isValidRollNo(token)) {
					printf("Invalid student roll no at line %d in input file, student roll no should be unique and alphanumeric\n", lineNo);
					return 1;
				}
			   ...
			   std->rollNo = strdup(token);
				...
				if (!isValidName(std->name)) {
					printf("Invalid student name at line %d in input file, student name should only contain alphabets\n", lineNo);
					return 1;
				}
			} 
				 ...
				 if (!isValidMinorMarks(atoi(token))) {
				 	printf("Invalid student minor marks at line %d in input file, student minor marks should be in the range 0-40\n", lineNo);
					return 1;
				}
				std->subj[(tflag-2)/3].minor = atoi(token);
			} 
			else if ((tflag-2)%3 == 2) { 	// major
				if (!isValidMajorMarks(atoi(token))) {
				 	printf("Invalid student major marks at line %d in input file, student major marks should be in the range 0-60\n", lineNo);
					return 1;
				}
				std->subj[(tflag-2)/3].major = atoi(token);
			}
			// printf("%d %s", tflag, token);
